...
export function isCapsCollection(collection: {
  handle: string;
  title?: string;
}): boolean {
  const value = `${collection.handle} ${collection.title ?? ""}`.toLowerCase();
  return ["baseball-cap", "baseball cap", "czapki", "czapka", "caps"].some(
